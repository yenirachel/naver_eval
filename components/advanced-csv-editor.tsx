...
export default function AdvancedCSVEditor() {
  ...
  const [expandedCell, setExpandedCell] = useState<{ rowIndex: number; header: string; content: string } | null>(null)
  ...
  const handleLLMEvaluationConfirm = (evaluationSettings: any) => {
  ...
  const handleCellClick = (rowIndex: number, header: string, content: string) => {
              ...
              <ResponsiveContainer width="100%" height="100%">
                {chartType === 'radar' ? (
                  <RadarChart data={calculateAverageScores(data, selectedVisualizationColumns)}>
                    ...
                    <PolarRadiusAxis angle={30} domain={[0, 7]} /><PolarRadiusAxis angle={30} domain={[0, 7]} />
